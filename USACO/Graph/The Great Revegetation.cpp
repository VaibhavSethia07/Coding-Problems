...
bool bipartite(int u,int col=1){
	color[u]=col;
	...
	for(pair<int,char> p:adj[u]){
		if(color[p.f]==0){
			if(p.s=='D')
				ok&=bipartite(p.f,3-color[u]);
			else
				ok&=bipartite(p.f,color[u]);
		}else{
			if(p.s=='S' && color[u]!=color[p.f])
				return false;
			if(p.s=='D' && color[u]==color[p.f])
				return false;
		}
	}
	return ok;
...
int main(){
	...
	for(int i=0;i<E;i++){
		cin>>ch>>a>>b;
		adj[a].push_back({b,ch});
		adj[b].push_back({a,ch});
	}
